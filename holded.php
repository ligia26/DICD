...
$dataFile = __DIR__ . "/holded_links.json"; 
if (!file_exists($dataFile)) {
    die("Fatal Error: /holded_links.json not found.");
}
$raw = file_get_contents($dataFile);
$COMPANY_LINKS = json_decode($raw, true);
if (!is_array($COMPANY_LINKS)) {
    die("Fatal Error: Invalid JSON in /holded_links.json");
}
...
<head>
  ...
  <title>Holded - Invoices and Billing</title>
  ...
  <style>
    ...
    .filters-section { 
        background:#fff; border-radius:10px; padding:1.5rem; margin-bottom:2rem; box-shadow:0 2px 8px rgba(0,0,0,0.1); 
    }
    .link-card { 
    ...
    .link-card:hover { 
        transform: translateX(4px); transition:.2s ease; box-shadow:0 4px 12px rgba(0,0,0,.08); 
    }
    .company { 
    ...
    .count-badge { 
        ...
        color:#fff; padding:.25rem .75rem; border-radius:20px; font-size:.85rem; font-weight:600; 
    }
</style>
</head>
<body>
    ...
    <div class="page-wrapper">
      <div class="page-content">
          ...
          <div class="ps-3">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Invoices and Billing</li>
              </ol>
            </nav>
          </div>
        </div>
        ...
        <div class="page-header">
          <h2 class="mb-1"><i class="fa-solid fa-file-invoice me-2"></i>Holded - Invoices and Billing</h2>
          <p class="mb-0 opacity-75">Access your company's invoice and billing details in Holded</p>
        </div>
        ...
        <?php else: ?>
          <div class="link-card">
            <div class="text-center text-muted py-3">
              ...
              <h5>No Holded Links Found</h5>
              <p>No links available for your company.</p>
            </div>
          </div>
        <?php endif; ?>
