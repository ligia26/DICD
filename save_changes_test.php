...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['data']) && isset($_POST['user_id'])) {
   ...
   // file_put_contents($log_file, file_get_contents('php://input'), FILE_APPEND);
    ...
    file_put_contents($log_file, "here is the sending domain" . $company_query . "\n", FILE_APPEND);
    

    file_put_contents($log_file, "here is the sending domain" . $sending_domain . "\n", FILE_APPEND);

    // Execute the query
    $company_result = $conn->query($company_query);

    file_put_contents($log_file, "STILL HERE \n", FILE_APPEND);


    $company = $company_result->fetch_assoc();
    $s3_dir = $company['s3_dir'];
    if (empty($s3_dir)) {
        die("S3 directory is empty, aborting S3 upload");
    }
    ...
    foreach ($data as $row) {
        echo "here is the man " . $row['manual_category'];

        if ($row['manual_category'] === 'Auto') {
            $manual_category = null;          // For database insertion
            $csv_manual_category = '';        // For CSV (empty value for S3)
        } else {
            $manual_category = $row['manual_category'] ?? null;
            $csv_manual_category = $row['manual_category'] ?? '';
        }
        $csvData[] = [
            $row['sending_domain'] ?? '',
            $row['user_domain'] ?? '',
    ...
    if (file_exists($tempFilePath)) {
        file_put_contents($log_file, "Temporary CSV file created: " . $tempFilePath . "\n", FILE_APPEND);
    } else {
        file_put_contents($log_file, "Failed to create temporary CSV file\n", FILE_APPEND);
        die("Failed to create temporary CSV file");
    }
    ...
    function uploadToS3($filePath, $bucket, $key) {
        ...
        file_put_contents($log_file, "I am runnung ", FILE_APPEND);
        ...
        try {
            ...
            file_put_contents($log_file, "S3 Upload Debug Info: " . json_encode($e->toArray()) . "\n", FILE_APPEND);
            return false;
        }
    ...
    file_put_contents($log_file, "Preparing to upload to S3. File path: " . $tempFilePath . " Bucket: " . $bucket . " Key: " . $key . "\n", FILE_APPEND);

    $uploadResult = uploadToS3($tempFilePath, $bucket, $key);

    IF(!$uploadResult) {

        file_put_contents($log_file, "STILL HERE 2 \n", FILE_APPEND);
    ...
    foreach ($data as $row) {
        ...
        $check_query = "SELECT COUNT(*) FROM config_changes WHERE sending_domain = '" . $conn->real_escape_string($sending_domain) . "' AND user_domain = '" . $conn->real_escape_string($user_domain) . "' AND country = '" . $conn->real_escape_string($country) . "'";
        ...
        // Execute the insert query
        if ($conn->query($insert_querys) === false) {
            $error = "Insert error in conf_changes_log: " . htmlspecialchars($conn->error);
            file_put_contents($log_file, $error . "\n", FILE_APPEND);
            echo $error;
        } else {
            file_put_contents($log_file, "Insert successful for conf_changes_log\n", FILE_APPEND);
            echo "Insert successful for conf_changes_log\n";
        }
