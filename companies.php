...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $athen_z =  '';
    $se_dir = $_POST['s3_dir'];
    $athen_z = $_POST['athena_name'];
    // Prepare SQL query
    
    // Check if company name already exists
    $check_company_sql = "SELECT id FROM companies WHERE name = ?";
    if ($stmt = $conn->prepare($check_company_sql)) {
        $stmt->bind_param("s", $name);
        $stmt->execute();
        $stmt->store_result();
        
        if ($stmt->num_rows > 0) {
            $_SESSION['error_message'] = "Company name already exists.";
        } else {
            // Prepare SQL query
            $sql = "INSERT INTO companies (name, status, s3_dir, athena_name) VALUES (?, ?, ?, ?)";

            if ($stmt = $conn->prepare($sql)) {
                // Bind parameters
                $stmt->bind_param("ssss", $name, $status ,$se_dir, $athen_z);
        ...
        // Close the check company statement
        $stmt->close();
    } else {
        $_SESSION['error_message'] = "Error preparing statement: " . $conn->error;
    }
                        ...
                        <div class="card">
                            <div class="card-body p-4">
                                ...
                                <form action="companies.php" method="post">
                                    ...
                                    <div class="row mb-3">
    <label for="inputS3Dir" class="col-sm-3 col-form-label">S3 Directory</label>
    <div class="col-sm-9">
        <input type="text" class="form-control" name="s3_dir" id="inputS3Dir" placeholder="S3 Directory">
    </div>
</div>
<div class="row mb-3">
    <label for="inputAthenaName" class="col-sm-3 col-form-label">Athena Name</label>
    <div class="col-sm-9">
        <input type="text" class="form-control" name="athena_name" id="inputAthenaName" placeholder="Athena Name">
    </div>
</div>
